...
BoatAnimation::BoatAnimation(TrackModel *track, BoatGraphicsItem *boat, int maxSize, QGraphicsItemAnimation *parent)
    ...
    setsailAt(0, model->getSailAngle(model->heading(), model->trim()));
    ...
    for (int i=0; i< size; i++) {
        ...
        setsailAt(index, model->getSailAngle(model->heading(), model->trim()));
        point = end;
    }
...
void BoatAnimation::afterAnimationStep(qreal step) {
    ...
    qreal sailAngle = m_boat->boat()->getSailAngle(heading, 0);
